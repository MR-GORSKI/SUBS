...
```
index.html             — единая entry-точка. Inline Root: auth-gate + DataLayer + viewport-роутер.
schema.sql             — DDL для Supabase (table + RLS + realtime). Запускается один раз в SQL editor.
supabase.js            — клиент Supabase + auth helpers + subsRepo (CRUD)
...
app.jsx                — десктопный App (pure renderer, props-driven). Экспортирует window.App.
```

**Порядок загрузки в `index.html`**:
1. React → ReactDOM → Babel → Supabase UMD
2. `supabase.js` — обычный (не Babel) скрипт, выставляет `sb`, `signInWithGoogle`, `signOut`, `userProfile`, `subsRepo`.
3. `tweaks-panel.jsx` → 4. `utils.jsx` → 5. `modals.jsx` → 6. `gantt.jsx` → 7. `auth.jsx` → 8. `app.jsx` → 9. `mobile.jsx`
10. Inline `<script type="text/babel">` объявляет `Splash`, `useAuth`, `AuthScreen`, `DataLayer`, `Root` и маунтит `<Root />`.

**Жизненный цикл `Root`**:
- `useAuth()` подписывается на `sb.auth.onAuthStateChange` → отдаёт `{user, loading}`.
- `loading` → `<Splash/>`.
- `!user` → `<AuthScreen/>` (Google-кнопка вызывает `signInWithGoogle()` → редирект к Google → возврат на ту же URL → Supabase ловит хеш → onAuthStateChange → user появляется).
- `user` → `<DataLayer user onSignOut isMobile/>`:
  - `subsRepo.list()` грузит подписки текущего юзера.
  - Realtime-channel ловит `postgres_changes` (insert/update/delete) и перезагружает.
  - `addSub/updateSub/deleteSub` — оптимистичные апдейты + сетевой запрос; на ошибке — alert + reload.
  - Рендерит `<App>` (desktop) или `<MobileApp>` (mobile) с props: `subs, addSub, updateSub, deleteSub, today, user, onSignOut`.
...
- [x] Персистентность: Supabase Postgres (`public.subscriptions`) с RLS по `auth.uid() = user_id`.
- [x] `today` = `new Date()` (нормализована на полдень).
- [x] Google OAuth через Supabase. Сессия персистится в `localStorage`, авто-refresh.
- [x] Realtime cross-device sync (postgres_changes на свою строку).
